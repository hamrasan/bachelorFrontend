...
%	otherlanguages = {czech},
	title-slovak = {Systém pre plánovanie a riadenie pestovateľského prostredia},
	title-czech = {Systém pro plánování a řízení pěstovatelského prostředí},
	title-english = {System for planning and managing of growing environment},
...
%	department-czech = {Katedra matematiky},
	department-slovak = {Katedra počítačov},
%	department-english = {Department of Mathematics},
	author = {Sandra Hamráková},
	supervisor = {Ing. Lukáš Zoubek},
%	supervisor-address = {Ústav X, \\ Uliční 5, \\ Praha 99},
	keywords-slovak = {internet vecí, inteligentná domácnosť, inteligentná záhrada, automatizované pestovanie},
	keywords-english = {Internet of Things, Smart Home, Smart Garden, automated cultivation},
	day = 8,
	month = 01,
	year = 2021,
...
% Abstract in Slovak
\begin{abstract-slovak}
V dnešnej dobe je takmer každé elektronické zariadenie možné pripojiť k internetu. Svet internetu vecí čím ďalej napreduje, preto je vhodné takého inteligentné riešenia využívať aj v záhradách. Práca poskytuje riešenie, ktoré pomôže drobným pestovateľom sprehľadniť pestovanie rastlín a automatizovať pestovateľské činnosti. Pomocou tohto systému dokážu pestovatelia zalievať záhradu prostredníctvom internetového pripojenia či získať informácie o vonkajšom prostredí konkrétnej záhrady pestovateľa. Sú to informácie o presnej teplote vzduchu, o aktuálnych zrážkach alebo o vlhkosti pôdy a vzduchu.
Táto práca sa zaoberá prieskumom existujúcich riešení systémov pre plánovanie a riadenie pestovateľského prostredia, následným návrhom a implementáciou takéhoto systému pomocou internetu vecí. Výsledné riešenie systému tvorí kombinácia webovej aplikácie spolu s mikročipom ESP8266, senzormi merajúcimi podmienky vonkajšieho prostredia a inteligentným zariadením, ktoré ovláda zalievanie záhrady.
\end{abstract-slovak}

% Abstract in English
\begin{abstract-english}
Nowadays, almost every electronic device is connected to the Internet. The world of the Internet of Things is advancing further, so it is appropriate to use such intelligent solutions in gardens. The work provides a solution that helps small growers to clarify plant cultivation and automate cultivation activities. With this system, growers can water the garden via the Internet connection or obtain information about the external environment of a particular grower's garden. These are information about the exact air temperature, current precipitation or soil and air humidity. This work deals with the research of existing systems for planning and managing the growing environment, the subsequent design and implementation of such a system, using the Internet of Things. The final solution of the system is a combination of a web application and the ESP8266 microchip, sensors measuring the conditions of the external environment, and an intelligent device that controls the garden's watering.
\end{abstract-english}

% Acknowledgements / Podekovani
\begin{thanks}
Ďakujem môjmu školiteľovi bakalárskej práce Ing. Lukášovi Zoubkovi, za ochotu, rady a odbornú pomoc. Taktiež ďakujem rodičom a priateľom, ktorí ma po celú dobu štúdia plne podporovali.
\end{thanks}
...
Testovacie scenáre \textit {\uv{Pridanie záhrady k senzoru na zalievanie}} a \textit {\uv{Odobratie záhrady zo senzoru na zalievanie}} po prvom otestovaní neprešli, kde záhrada sa nepridala ani neodobrala zo zoznamu záhrad v senzore na zalievanie. Po následnej oprave chyby všetkých 11 testov úspešne prešlo. 
\newline 
\newline 
...
\begin{figure}[h]
\fbox{\includegraphics[width= 12 cm]{domov.png}}
\caption{Domovská stránka aplikácie}
\label{fig:domov}
\end{figure}

Po prihlásení do aplikácie sa užívateľovi zobrazí domovská obrazovka, viď obrázok ~\ref{fig:domov}, s jeho jednotlivými záhradami. Každá záhrada obsahuje zoznam senzorov a hodnoty nimi namerané. Ak žiadne meranie zatiaľ neprebehlo alebo senzory nie sú pripojené, tak sa pri jednotlivom senzore zobrazí nápis  \textit {\uv{Senzor nie je dostupný}}. V prípade, že nameraných hodnôt na jednotlivých senzoroch je viac ako 6, je možné si zobraziť plnú históriu meraní po stlačení na tlačídko  \textit {\uv{Celá história}}. Časový interval merania senzorov je možné zmeniť v ľavom hornom rohu obrazovky.

\begin{figure}[H]
\fbox{\includegraphics[width= 12 cm]{mojaZahrada.png}}
\caption{Stránka aplikácie \uv{Moja záhrada} }
\label{fig:zahrada}
\end{figure}

Po kliknutí na odkaz v navigácií \textit {\uv{Moja záhrada}}, sa zobrazia všetky pestované rastliny v jednotlivých záhradách užívateľa, čo ukazuje obrázok ~\ref{fig:zahrada}. Po kliknutí na tlačídko \textit {\uv{Pridať záhradu}} si užívateľ môže vytvoriť novú záhradu, ktorá sa mu následne zobrazí v zozname pod daným tlačídkom.
Užívateľ je schopný filtrovať výber zobrazených rastlín podľa kategórie a taktiež vyhľadať rastlinu podľa názvu pomocou vyhľadávacieho okna vedľa filtra. Kliknutím na jednu z rastlín, sa zobrazí podstránka s náhľadom rastliny, viď obrázok ~\ref{fig:nahladRastliny}. V náhľade sa zobrazujú hodnoty rastliny, ktoré si užívateľ môže upraviť alebo môže vymazať celú rastlinu zo svojej záhrady. Pre pridanie novej rastliny do konkrétnej záhrady musí mať užívateľ zakliknutú záhradu, kde chce rastlinu umiestniť a následne kliknúť na tlačídko  \textit {\uv{Pridaj novú rastlinu}}, ktoré ho presmeruje na formulár pre pridanie rastliny viď obrázok ~\ref{fig:pridanieRastliny}.


\begin{figure}[H]
\fbox{\includegraphics[width=12 cm]{nahladRastliny.png}}
\caption{Detail rastliny}
\label{fig:nahladRastliny}
\end{figure}

\begin{figure}[H]
\fbox{\includegraphics[width= 12 cm]{pridanieRastliny.jpg}}
...
\begin{figure}[H]
\fbox{\includegraphics[width= 12 cm]{zalievanie.png}}
\caption{Stránka aplikácie \uv{Zalievanie}}
\label{fig:zalievanie}
\end{figure}

Po kliknutí na odkaz v navigácií\textit {\uv{Zalievanie}}, sa zobrazí podstránka s náhľadom jednotlivých zalievačov užívateľa. Užívateľ si môže pridať polievač pomocou tlačídka \textit {\uv{Pridaj polievač}}, kde zadá platné id senzora. Po úspešnom pridaní sa polievač objaví v zozname. Užívateľ si môže na každom polievači nastaviť rozvrh na zalievanie alebo nastaviť trvanie zálievky a poliať záhradu ihneď. V zozname \textit {\uv{V záhrade}} sa vypíšu záhrady priradené k tomuto polivaču, ktoré užívateľ môže regulovať tlačídkom \textit {\uv{Pridať/odobrať záhradu}}.

\begin{figure}[H]
\fbox{\includegraphics[width= 12 cm]{notifikacie.png}}
\caption{Stránka aplikácie \uv{Notifikácie}}
\label{fig:notifikacie}
\end{figure}

V neposlednom rade tu máme notifikácie aplikácie, ktoré sa zobrazujú po kliknutí na \textit {\uv{Notifikácie}} v navigácií aplikácie.  Každá notifikácia sa zobrazuje, kým užívateľ nepotvrdí, že notifikáciu videl, zakliknutím ikonky fajky, vpravo vedľa notifikácie. Užívateľ si tu môže nastaviť, kedy ho má aplikácia upozorniť, pri poklese alebo náraste nad určitú teplotu.
\chapter{Záver}
...
\label{sec:scenare}
\includepdf[pages=-]{testovacieScenare.pdf}
...
\bibitem{react} AGGARWAL, Sanchit. Modern Web-Development using ReactJS. In:  \textit{International Journal of Recent Research Aspects.} Vol.5. India: Institute of Institutional Industrial Research, 2018, s. 133-137. ISSN 2349-7688.
\end{thebibliography}
